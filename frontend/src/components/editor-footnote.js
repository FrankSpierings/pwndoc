...
const Footnote = Node.create({
  ...
  renderHTML({ HTMLAttributes }) {
    let content;
    if (HTMLAttributes.content.startsWith("http")) {
      content = ['a', { href: HTMLAttributes.content, target: "_blank" }, HTMLAttributes.id.toString()];
    } else {
      content = HTMLAttributes.id.toString();
    }
    return ['sup', { 'data-footnote': HTMLAttributes.id, title: HTMLAttributes.content }, content];
  },
