...
export default {
  htmlEncode(html) {
    ...
    DOMPurify.addHook('uponSanitizeAttribute', function (node, data, config) {
      ...
      // Filter authorized attributes for <table>-related tags (style)
      else if (node.tagName === 'TABLE' || node.tagName === 'COLGROUP' || node.tagName === 'COL' || node.tagName === 'TBODY' || node.tagName === 'THEAD') {
        if (data.attrName === 'style') {
          data.forceKeepAttr = true;
        }
      }
      // Filter authorized attributes for <td> and <th> tags (style, colspan, rowspan, colwidth)
      else if (node.tagName === 'TD' || node.tagName === 'TH') {
        if (data.attrName === 'style' || data.attrName === 'colspan' || data.attrName === 'rowspan' || data.attrName === 'colwidth') {
          data.forceKeepAttr = true;
        }
      }
      // Filter authorized attributes for <sup> tags (<sup data-footnote="..." title="...">)
      else if (node.tagName === 'SUP') {
        if (data.attrName === 'data-footnote' || data.attrName === 'title')
          data.forceKeepAttr = true;
      }
    });
