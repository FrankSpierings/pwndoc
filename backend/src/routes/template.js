module.exports = function (app) {
    ...
    app.post("/api/templates", acl.hasPermission('templates:create'), function (req, res) {
        ...
        Template.create(template)
            .then(data => {
                ...
                fs.writeFileSync(`${__basedir}/../report-templates/${path.basename(template.name)}.${path.basename(template.ext)}`, fileBuffer);
        ...
        Template.update(req.params.templateId, template)
            .then(data => {
                ...
                if (req.body.name && !req.body.file) {
                    fs.renameSync(`${__basedir}/../report-templates/${path.basename(data.name)}.${path.basename(data.ext || 'docx')}`,
                        `${__basedir}/../report-templates/${path.basename(req.body.name)}.${path.basename(data.ext || 'docx')}`);
                }
                // Update both name and file
                else if (req.body.name && req.body.file && req.body.ext) {
                    var fileBuffer = Buffer.from(req.body.file, 'base64');
                    try { fs.unlinkSync(`${__basedir}/../report-templates/${path.basename(data.name)}.${path.basename(data.ext || 'docx')}`) } catch { }
                    fs.writeFileSync(`${__basedir}/../report-templates/${path.basename(req.body.name)}.${path.basename(req.body.ext)}`, fileBuffer);
                }
    ...
    app.delete("/api/templates/:templateId", acl.hasPermission('templates:delete'), function (req, res) {
        Template.delete(req.params.templateId)
            .then(data => {
                fs.unlinkSync(`${__basedir}/../report-templates/${path.basename(data.name)}.${path.basename(data.ext || 'docx')}`)
    ...
    app.get("/api/templates/download/:templateId", acl.hasPermission('templates:read'), function (req, res) {
        Template.getOne(req.params.templateId)
            .then(data => {
                var file = `${__basedir}/../report-templates/${path.basename(data.name)}.${path.basename(data.ext || 'docx')}`
