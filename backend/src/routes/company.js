module.exports = function (app) {
    ...
    app.put("/api/companies/:id", acl.hasPermission('companies:update'), function (req, res) {
        var company = {};
        // Optional parameters
        if (req.body.name) company.name = String(req.body.name);
        if (req.body.shortName) company.shortName = String(req.body.shortName);
        if (req.body.logo) company.logo = String(req.body.logo);
